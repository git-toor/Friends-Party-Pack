...
class DiceBox {
	...
	setDimensions(dimensions) {
		...
		// Match DicePreview lighting: clean white AmbientLight + DirectionalLight
		if (this.light) this.scene.remove(this.light);
		if (this.light_amb) this.scene.remove(this.light_amb);
		if (this.light_rim) this.scene.remove(this.light_rim);
		this.light_amb = new THREE.AmbientLight(0xffffff, 0.7);
	...
	clearDice() {
		...
		while (dice = this.diceList.pop()) {
			...
			// Dispose Three.js resources to prevent memory leak
