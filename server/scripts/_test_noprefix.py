...
tests = [
    ("dwight_office", "Dwight Schrute as a cartoon character with cat ears and whiskers, wearing yellow mustard short sleeve button up shirt, brown pants, brown tie, round wire glasses, side parted brown hair, smug expression, standing in an office with desk and computer, clean flat cartoon art style, board game illustration, white background"),

    ("ross_couch", "Ross Geller as a cartoon character with cat ears and whiskers, wearing a brown leather vest over a light shirt, dark jeans, mid-length wavy dark hair, angry shouting expression, sitting on an orange couch in a coffee shop living room, clean flat cartoon art style, board game illustration, white background"),
]
...
for name, prompt_text in tests:
    ...
    workflow = {
        ...
        "neg": {"class_type": "CLIPTextEncode", "inputs": {"text": "sketch, rough, unfinished, messy lines, photorealistic, 3d render, bad anatomy, blurry, low quality, orange cat, fat cat, generic cat", "clip": ["dual_clip", 0]}},
        "empty": {"class_type": "EmptyLatentImage", "inputs": {"width": 768, "height": 1024, "batch_size": 1}},
        "ksampler": {"class_type": "KSampler", "inputs": {"seed": 9995 if name == "dwight_office" else 9994, "steps": 28, "cfg": 1.0, "sampler_name": "euler", "scheduler": "normal", "denoise": 1.0, "model": ["ckpt", 0], "positive": ["pos", 0], "negative": ["neg", 0], "latent_image": ["empty", 0]}},
